...
public class ListaEncadeadaImpl<T extends Comparable<T>> implements ListaEncadeadaIF<T> {
    ...
    public NodoListaEncadeada<T> cabeca;
    public NodoListaEncadeada<T> cauda;
    ...
    // NOVO METODO
    public NodoListaEncadeada<T> removeInicio() throws ListaVaziaException {
        if (isEmpty()) throw new ListaVaziaException();
        NodoListaEncadeada<T> primeiro = cabeca.getProximo();
        cabeca.setProximo(primeiro.getProximo());
        tamanho--;
        NodoListaEncadeada<T> retorno = new NodoListaEncadeada<T>(primeiro.getChave());
        if (primeiro.getProximo() != cauda) {
            retorno.setProximo(new NodoListaEncadeada<T>(primeiro.getProximo().getChave()));
        }
        return retorno;
    }
    /**
     * Insere um novo nó com a chave especificada no final da lista.
     * O novo nó é adicionado antes do nó sentinela cauda.
     */
